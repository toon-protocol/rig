...
- 9ce96bc: Relay-native CI (#125): `rig ci`, a coordinator, and NIP-C1 events.
...
- 9a585f6: Size is priced, not refused; and `rig chain set mina` is actually refused
  ...
  **Object size (#102).** `MAX_OBJECT_SIZE` was `95 * 1024` (97,280), described as
  ...
  Worse, it was a _hard error_: an object over it aborted the whole push. That
...
- 74800c0: `rig name buy` (direct, wallet-paid path): spawn our own ANT before registering, and never exit 0 with no ANT attached.
...
- 1f34792: factory-job proof: resume the provider's existing channel instead of locking a fresh deposit every run.
...
- a905e38: chore(deps): `@toon-protocol/client` `^0.29.0`, `@toon-protocol/core` `^3.2.0`
...
- 293dc14: Wire `JobDeliveryPort` to the released `@toon-protocol/client` (#56): the last piece of toon-meta#262's "agents earning" epic — a real, runnable proof that one paid factory-job increment moves end to end.
...
- 3125338: Consume `@toon-protocol/client@^0.25.0`, so a published rig can do the Solana greeting-bootstrap.
...
- 0eff75a: The official Rust edge is rig's default uplink.
...
- 3fe889a: Move onto `@toon-protocol/client` 0.22.0: the sealed wire, and a flat price
