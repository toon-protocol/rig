...
import { RunStateBadge, TrustBadge, workflowLabel } from '@/app/pages/actions-page';
  ...
  return (
    <section className="rounded-md border">
      <header className="flex flex-wrap items-center gap-2 border-b bg-muted/40 px-4 py-2">
        <h3 className="font-semibold text-foreground">{job.name ?? job.jobId}</h3>
        {job.name && job.name !== job.jobId && (
          <span className="font-mono text-xs text-muted-foreground">{job.jobId}</span>
        )}
        <Badge className={`text-[10px] ${conclusionBadgeClass('concluded', job.conclusion)}`}>
        ...
        {job.runsOn.length > 0 && (
          <span className="text-xs text-muted-foreground">on {job.runsOn.join(', ')}</span>
        )}
        <span className="ml-auto text-xs text-muted-foreground">
          {job.queuedAt !== undefined && <span title={formatClock(job.queuedAt)}>queued {formatRelativeDate(job.queuedAt)} · </span>}
          {job.startedAt !== undefined && <span title={formatClock(job.startedAt)}>started {formatRelativeDate(job.startedAt)}</span>}
      ...
      <footer className="flex flex-wrap items-center gap-3 border-t px-4 py-2 text-xs">
        {job.logsUrl ? (
          <a href={job.logsUrl} target="_blank" rel="noreferrer" className="text-primary underline-offset-2 hover:underline">
            Full log
          </a>
        ) : (
            ...
            {job.artifacts.map((artifact, i) => (
              <span key={artifact.url}>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  title={artifact.name ? `artifact "${artifact.name}"` : undefined}
...
function PendingJob({ jobId, running }: { jobId: string; running: boolean }) {
  return (
    <section className="flex items-center gap-2 rounded-md border border-dashed px-4 py-3">
      ...
      <Badge className={`text-[10px] ${conclusionBadgeClass(running ? 'in_progress' : 'queued', undefined)}`}>
...
function RunHeader({ run, owner, repo }: { run: CiRun; owner: string; repo: string }) {
  const { getDisplayName } = useProfileCache();
  const duration =
    run.status === 'concluded' && run.startedAt !== undefined && run.createdAt >= run.startedAt
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
        ...
        {run.trigger.ref && (
          <span className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">{shortRefName(run.trigger.ref)}</span>
        )}
        {run.trigger.pr && (
          <Link
      ...
      <p className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
        <span>
          coordinator{' '}
          <Link to={`/${hexToNpub(run.coordinator)}`} className="text-foreground hover:text-primary hover:underline">
            {getDisplayName(run.coordinator)}
          </Link>
        </span>
        {run.queuedAt !== undefined && <span title={formatClock(run.queuedAt)}>· queued {formatRelativeDate(run.queuedAt)}</span>}
        {run.startedAt !== undefined && <span title={formatClock(run.startedAt)}>· started {formatRelativeDate(run.startedAt)}</span>}
        {duration && <span>· took {duration}</span>}
        {run.queue !== undefined && run.status === 'queued' && <span>· {run.queue} round{run.queue === 1 ? '' : 's'} ahead</span>}
        {run.provenance && (
...
export function RunDetailPage() {
  ...
  const { run, jobs, loading, error } = useCiRun(owner, metadata.repoId, runId, metadata.maintainers);
  ...
  if (error) {
    return <div className="text-destructive-foreground">Failed to load the run: {error.message}</div>;
  }
  ...
  return (
    <div className="space-y-4">
      <Link to={`/${owner}/${repo}/actions`} className="text-xs text-muted-foreground hover:text-primary hover:underline">
        ← All workflow runs
      </Link>
      ...
      <div className="space-y-3">
        ...
        {pendingIds.map((id) => (
          <PendingJob key={id} jobId={id} running={run.inProgress.includes(id)} />
        ))}
