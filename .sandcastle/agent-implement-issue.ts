...
//                             a dead credential — see toon-meta#248 and
//                             ./mint-app-token.ts. HOST ONLY: deliberately
//                             absent from PASSTHROUGH_KEYS in
//                             ./sandbox-secrets.ts, so the private key never
//                             enters the sandbox container. Optional —
//                             without it the runner falls back to the
//                             ambient GH_TOKEN and the old expiry behaviour.
//
// Usage:
...
// rig is a pnpm workspace: install with the committed lockfile so the sandbox
...
// (toon-meta#248, porting connector#463)
// ---------------------------------------------------------------------------
...
// `credential.helper=` that precedes it on the command line is load-bearing:
// git treats credential.helper as a MULTI-VALUED config key and an empty
// value RESETS the list, which is what stops `gh auth setup-git`'s
// container-global helper (wired in the onSandboxReady hook above, and
// holding the STALE token from container start) from being consulted first
// and winning.
//
 ...
 *
 * Also refreshes the HOST's `GH_TOKEN` from the same mint, because the host
 * `gh` calls that follow (`pr list`, `pr create`, `api`) authenticate from
 * `process.env` and expire on exactly the same one-hour clock.
 *
 * `bestEffort` is for the early publish after the implementer phase: a
 * failure there costs us recoverability but must not abandon a run that
 * still has a review phase to do. The final push is never best-effort — it
 * fails loud.
 */
async function pushBranch(
  ...
  // ONE place decides what a failure MEANS, so every step below can just
  // report what went wrong: best-effort warns and returns false, otherwise we
  // throw and the run fails loud.
  ...
  try {
    ...
    console.log(
      `  [${label}] credential: freshly minted (source=${minted.source})`,
    );
  } catch (err) {
    return fail(
      `could not obtain a push credential: ${(err as Error).message}`,
    );
  }
  ...
  try {
    ...
    // phases that follow. Inside the `try` that starts at STAGING, so a
    // half-written file from a failed stage is cleaned up too.
...
try {
  ...
  // PUBLISH EARLY (toon-meta#248, porting connector#463). The implementer has
  ...
  //      cancellation, runner death) leaves the completed implementation on
  //      a remote branch instead of losing it with the container.
  //   2. It is the cheapest moment to fail: if push auth is broken we learn
  //      it here, minutes in, not near the end of the job.
  ...
  if (autoMerge && !blocking) {
    ...
    // gh pr create are pure plumbing: push from INSIDE the sandbox, open the
    // PR from the authenticated HOST. sandbox.exec() surfaces a non-zero
    // exitCode (it does NOT throw) — check it and fail loud.
    //
    // The credential is minted fresh here, immediately before the push, so
    // the run's total length is irrelevant (toon-meta#248). This also
