...
// (`actions/create-github-app-token@v2`) and the runner pushed only after the
// implementer AND the reviewer had both finished. Any run longer than an hour
// therefore died at the very last step:
//
...
//
// In connector this destroyed three completed implementations (#430 at 77
// min, #422 twice at 61 and 73 min) — the failure lands after all the
// expensive work is done. Raising `timeout-minutes` on its own makes this
// worse, not better: the extra minutes are spent and the push still fails.
//
// THE FIX
// -------
...
// because the push happens from INSIDE the sandbox, part-way through
// agent-implement-issue.ts's execution — there is no workflow step boundary
// at that moment to hang an action off. See agent-implement-issue.ts's
// pushBranch() for how the minted token is handed to git without ever
// appearing in argv or in the logs.
//
...
// The expiry problem is a CI-long-run problem; a local run already has a
// token in the env and no way to mint.
 ...
 * runner and GitHub, which is the documented recommendation. Exported for
 * ./gate/mint-app-token.test.ts, which verifies the claims and signature
 * shape without a real App key.
 */
 ...
 * since every caller needs *some* credential.
 */
