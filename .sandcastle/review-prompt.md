...
formatting, redundant code, unclear abstractions, missing obvious edge-case
handling that does not change intended behaviour.
...
- the diff asserts something factually wrong (in code, comments, or docs)
  that you cannot verify a fix for
...
2. Run rig's gate to ensure nothing is broken — `eslint .`, `pnpm run typecheck`, `pnpm -r test --if-present`, and `pnpm -r build`
