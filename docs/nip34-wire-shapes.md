...
| `relays`                                                                             | optional, multi-value                       | The relay URL(s) _this publish_ is going to, written by `rig push`, `rig maintainers`, `rig payout`, `rig refresh`. Rewritten (not merged) on every owner-initiated republish, so it always reflects the most recent publish.                                                      |
| `web`                                                                                | optional, multi-value                       | Rig writes exactly one value: the repo's rig-web viewer URL, built by `repoWebUrl()` (`rig-pointer.ts`) from the same base URL and route shape as the Rig pointer's own fallback link — governed by ADR-0001's URL-permanence rules. Rewritten on every owner-initiated republish. |
